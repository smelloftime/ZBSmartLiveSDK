...
@implementation LRResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(FMStatement *)statement usingParentDatabase:(LRDatabase*)aDB {
    
    LRResultSet *rs = [[LRResultSet alloc] init];
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"LRDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(LRDatabase *)newDb {
    _parentDB = newDb;
}
