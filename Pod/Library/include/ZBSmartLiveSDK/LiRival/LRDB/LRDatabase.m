...
@interface LRDatabase ()

- (LRResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation LRDatabase
...
#pragma mark LRDatabase instantiation and deallocation
...
// if ([LRDatabase FMDBVersion] >= 0x0240) { … }
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    LRDatabase *self = (__bridge LRDatabase*)f;
...
// but for folks who don't bother noticing that the interface to LRDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        LRResultSet *rs = (LRResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(LRResultSet *)resultSet {
...
- (BOOL)goodConnection {
    ...
    LRResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The LRDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The LRDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The LRDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The LRDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"LRDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (LRResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (LRResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    rs = [LRResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (LRResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (LRResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (LRResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (LRResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    LRResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (LRResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (NSError*)inSavePoint:(void (^)(BOOL *rollback))block {
    ...
    NSString *errorMessage = NSLocalizedString(@"Save point functions require SQLite 3.7", nil);
    if (self.logsErrors) NSLog(@"%@", errorMessage);
    return [NSError errorWithDomain:@"LRDatabase" code:0 userInfo:@{NSLocalizedDescriptionKey : errorMessage}];
#endif
}
