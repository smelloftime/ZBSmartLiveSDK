...
#import "LRDatabasePool.h"
#import "LRDatabase.h"

@interface LRDatabasePool()

- (void)pushDatabaseBackInPool:(LRDatabase*)db;
- (LRDatabase*)db;

@end


@implementation LRDatabasePool
...
- (void)pushDatabaseBackInPool:(LRDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The LRDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (LRDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [LRDatabase databaseWithPath:self->_path];
