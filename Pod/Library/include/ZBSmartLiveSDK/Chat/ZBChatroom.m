...
- (void)leaveChatroom:(NSNumber *)chatroomID password:(NSString *)password completion:(void (^)(id, NSError *))completion {
    ...
    NSDictionary *messageModelDic = [messageModel mj_JSONObject];
    /** 首先发送一条 离开聊天室的消息, 然后再发送 离开聊天室这个事件 */
    [self.liRivalKit sendMessageWithMessageEvent:ZBConversationEventMessage messageBody:messageModelDic messageIdentity:@(self.liRivalKit.sendMessageIdentity) completion:^(NSArray *respondData, NSError *error) {
        if (completion) {
            if (error) {
                completion(nil, error);
            } else {
